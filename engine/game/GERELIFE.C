...
void DoLife(WORD numobj)
{
	...
	while (flag != -1)
	{
		...
		case LM_SET_COMPORTEMENT:
			ptrobj->OffsetLife = LE_R16(PtrPrg);
			PtrPrg += 2;
			break;
		...
		case LM_SET_COMPORTEMENT_OBJ:
			num = *PtrPrg++;
			ListObjet[num].OffsetLife = LE_R16(PtrPrg);
			PtrPrg += 2;
			break;
		...
		case LM_SET_TRACK:
			if (HasLoadedListObjetTracksOnSave)
			{
				if (ptrobj->OffsetTrack == -1)
					ptrobj->OffsetTrack = LE_R16(PtrPrg);
			}
			else
				ptrobj->OffsetTrack = LE_R16(PtrPrg);
			PtrPrg += 2;
		...
		case LM_SET_TRACK_OBJ:
			...
			{
				if (ptrobj->OffsetTrack == -1)
					ListObjet[num].OffsetTrack = LE_R16(PtrPrg);
			}
			else
				ListObjet[num].OffsetTrack = LE_R16(PtrPrg);
			PtrPrg += 2;
		...
		case LM_IF:
			...
			if (!DoTest())
			{
				PtrPrg = ptrobj->PtrLife + LE_R16(PtrPrg);
			}
			else
			{
				PtrPrg += 2;
		...
		case LM_SET_DOOR_LEFT:
			ptrobj->Beta = 768;
			ptrobj->PosObjX = ptrobj->AnimStepX - LE_R16(PtrPrg);
		...
		case LM_SET_DOOR_UP:
			ptrobj->Beta = 512;
			ptrobj->PosObjZ = ptrobj->AnimStepZ - LE_R16(PtrPrg);
			ptrobj->WorkFlags &= ~AUTO_STOP_DOOR;
			ptrobj->SRot = 0;
			PtrPrg += 2;
			break;
		...
		case LM_ADD_CHOICE:
			GameListChoice[GameNbChoices] = LE_R16(PtrPrg);
			PtrPrg += 2;
			GameNbChoices++;
			break;
