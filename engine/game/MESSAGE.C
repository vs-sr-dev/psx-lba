...
/* PORT: UBYTE, not char. We build -fsigned-char (the 1994 x86 code assumes
 * it), so *PtLine sign-extends every byte from 0x80 up: the "ù" of "Book of
 * Bù" is 0x97 and arrived at CarFont as -105, which indexed 420 bytes below
 * the font offset table and produced a wild glyph pointer. T_STACKCAR.c is a
 * signed WORD too, so the sign survived the whole way down. Accented text is
 * the norm in fr/de/es/it, so this fired far beyond one string. */
static UBYTE *PtLine;
...
void DeleteOlderFileHD()
{
	...
	if (FileSize(pt1->NameHD))
		Delete(pt1->NameHD); //  cf Del upper
...
/*-------------------------------------------------------------------------*/
/* PORT: the body was commented out in the 1994 source, which makes this a
 * no-op — and InitFileNar() calls it immediately before OpenRead(), so every
 * voice-bank init leaked a handle and nothing ever closed FdNar on the normal
 * path. DOS tolerated that; a port with a bounded open-file table does not.
 * Once the table filled, the next archive to want opening was refused, and
 * HQR_Get's own "if (!handle) return 0" then handed OBJECT.C:3024 a null
 * animation pointer to dereference — the guru at SetInterAnimObjet2+0xc.
 * Restored exactly as the author wrote it; his comment says as much. */
void CloseFdNar()
{
...
void InitFileNar(char *file, LONG from)
{
	...
	while (!offset) // Sans Filet
	{
		...
		/* PORT: "sans filet" indeed — on a truncated or empty bank Read()
		 * keeps returning 0 with `offset` untouched and this spins forever.
		 * Harmless while the VOX sat in the read-only nitroFS; the banks now
		 * come off a user-copied SD, where a short/aborted copy is a real
		 * possibility. Bail out and let the caller run without voices.
		 *
		 * NB the comparison is against 1, not 4: Read() (FILES.C) is
		 * fread(buffer, lenread, 1, handle), so it returns the ITEM count,
		 * never the byte count. Testing "wr != 4" made this fire on every
		 * healthy bank and killed all speech. The engine's own check further
		 * down ("wr != offset") has the same units confusion, but is
		 * harmless there because CloseFdNar() is commented out in this
		 * source — which is also why this path has to clear FdNar itself,
		 * or the "if (!FdNar)" guards in PlaySpeak/TestSpeak would let a
		 * half-initialised BufMemoSeek through. */
		if (wr != 1)
		{
			if (FdNar)
			{
	...
	/* PORT: and the seek table is read straight into BufMemoSeek, which is a
	 * fixed 2048-byte SmartMalloc (PERSO.C:1558) — a corrupt header would
	 * overflow the heap. Same family as the BufOrder overflow. */
	if (offset > 2048)
	{
		if (FdNar) /* CloseFdNar() is a no-op here — see above */
		{
			Close(FdNar);
			FdNar = 0L;
		}
...
{
	if (!FlagSpeak)
		return (0); //     Le joueur ne veut pas
	if (!fd)
		return (0); //     Le programme ne peut pas
...
// #else
	...
	MaxText = Load_HQR(FileText, BufOrder,
					   (Language * MAX_TEXT_LANG * 2) + (file * 2) + 0) /
			  2;
...
void GetNextLine()
{
	...
	*BufLine = 0;
	...
	while (TRUE)
	{
		...
		if (SizeLine + SizeSpace + SizeWord > DialMaxSize)
			break;
