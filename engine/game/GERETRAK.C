...
void DoTrack(WORD numobj)
{
	...
	while (flag)
	{
		...
		switch (macro)
		{
			/*-------------------------------------------------------------------------*/
		case TM_SAMPLE:
			HQ_3D_MixSample(LE_R16(ptrtrack), 0x1000, 1,
							ptrobj->PosObjX, ptrobj->PosObjY, ptrobj->PosObjZ);
			ptrobj->OffsetTrack += 2;
			break;

		case TM_SAMPLE_RND:
			HQ_3D_MixSample(LE_R16(ptrtrack), 0x800 + Rnd(0x800), 1,
							ptrobj->PosObjX, ptrobj->PosObjY, ptrobj->PosObjZ);
			ptrobj->OffsetTrack += 2;
			break;

		case TM_SAMPLE_ALWAYS:
			HQ_3D_MixSample(LE_R16(ptrtrack), 0x1000, 0,
							ptrobj->PosObjX, ptrobj->PosObjY, ptrobj->PosObjZ);
			ptrobj->OffsetTrack += 2;
			break;

		case TM_SAMPLE_STOP:
			HQ_StopOneSample(LE_R16(ptrtrack));
			ptrobj->OffsetTrack += 2;
			break;

		case TM_REPEAT_SAMPLE:
			BigSampleRepeat = LE_R16(ptrtrack);
			ptrobj->OffsetTrack += 2;
			break;

		case TM_SIMPLE_SAMPLE:
			HQ_MixSample(LE_R16(ptrtrack), 0x1000, BigSampleRepeat, 128, 128);
			BigSampleRepeat = 1;
			ptrobj->OffsetTrack += 2;
			break;
		...
		case TM_WAIT_NB_SECOND:
			...
			if ((ULONG)LE_R32(ptrtrack + 1) == 0) /* PORT: odd offset — LE access, cf. DEFINES.H */
			{
				LE_W32(ptrtrack + 1, TimerRef + (*ptrtrack * 50));
			}

			if (TimerRef < (ULONG)LE_R32(ptrtrack + 1))
			{
		...
		case TM_OPEN_DOWN:
			...
			if ((ptrobj->Flags & (SPRITE_3D + SPRITE_CLIP)) == SPRITE_3D + SPRITE_CLIP)
			{
				...
				ptrobj->DoorWidth = LE_R16(ptrtrack);
		...
		case TM_LABEL:
			...
			/*-------------------------------------------------------------------------*/
		case TM_GOTO:
			ptrobj->OffsetTrack = LE_R16(ptrtrack);
			break;
