...
void HitObj(WORD numhitter, WORD num, WORD hitforce, WORD beta)
{
	...
	if (ptrobjt->LifePoint <= 0 || (num == NUM_PERSO && PersoInvulnerable))
		return;
	...
	if (ptrobjt->Armure <= hitforce)
	{
		...
		{
			/* PORT: seul appelant de GereAnimAction sans ce test (les 3 autres
			   sites l'ont) ; PtrAnimAction vaut 0 des que l'anim en cours n'a
			   pas de bloc d'action -> deref de 0 dans GereAnimAction. */
...
LONG CheckValidObjPos(WORD numobj)
{
	...
	if ((x0 < 0) OR(x0 > SIZE_BRICK_XZ * 63))
		return FALSE;
	if ((x1 < 0) OR(x1 > SIZE_BRICK_XZ * 63))
		return FALSE;
	if ((z0 < 0) OR(z0 > SIZE_BRICK_XZ * 63))
		return FALSE;
	if ((z1 < 0) OR(z1 > SIZE_BRICK_XZ * 63))
		return FALSE;

	// test decors

	if (WorldColBrickFull(x0, y0, z0, ptrobj->Ymax))
		return FALSE;
	if (WorldColBrickFull(x1, y0, z0, ptrobj->Ymax))
		return FALSE;
	if (WorldColBrickFull(x1, y0, z1, ptrobj->Ymax))
		return FALSE;
	if (WorldColBrickFull(x0, y0, z1, ptrobj->Ymax))
		return FALSE;
	...
	//	if( (newanim != ptrobj->Anim) OR (flag != ptrobj->FlagAnim) )
	{
		...
		*/
		...
		else
		{
			...
			if (PtrBufferAnim > BufferAnim + 5000 - 512)
				PtrBufferAnim = BufferAnim;
		}
...
void ReajustPos(UBYTE col)
{
	...
	case 8:
		if (SIZE_BRICK_XZ - (Nxw - xw) > Nzw - zw)
			col = 4;
		else
			col = 2;
		break;
...
void ReceptionObj()
{
	if (AnimNumObj == NUM_PERSO)
	{
		...
		else if (StartYFalling - Nyw >= 8 * SIZE_BRICK_Y)
		{ // se fait mal
			InitSpecial(APtObj->PosObjX,
						APtObj->PosObjY + 1000,
						APtObj->PosObjZ,
...
void DoCornerReajust(WORD nx, WORD ny, WORD nz, WORD coin)
{
	...
	if (Nzw < 0)
		goto fincorner;
	if (Nxw > 63 * SIZE_BRICK_XZ)
		goto fincorner;
	if (Nzw > 63 * SIZE_BRICK_XZ)
		goto fincorner;
...
void DoAnim(WORD numobj)
{
				...
				if (APtObj->WorkFlags & AUTO_STOP_DOOR)
				{
					...
					if (APtObj->DoorWidth)
					{
						...
						if (Distance2D(Nxw,
									   Nzw,
									   APtObj->AnimStepX,
									   APtObj->AnimStepZ) >= APtObj->DoorWidth)
						{
							// recalage violent
							switch (APtObj->Beta)
							{
	...
	/* gestion anim/DEP OBJET 3D */
		...
		/* Body != -1 is an invariant of the anim system — InitAnim refuses
		   "Anim sur un objet avec Body = -1 !" (OBJECT.C:1173). But the death
		   and spawn transitions (GERELIFE LM_KILL_OBJ/LM_SUICIDE/
		   LM_INIT_PINGOUIN, the NPC death path in PERSO.C, DISKFUNC load) set
		   Body = -1 WITHOUT clearing Anim. A dead object is normally skipped
		   at the top of the PERSO.C object loop (OBJ_DEAD -> continue), but if
		   one reaches DoAnim while Body == -1 and Anim != -1 (mid-frame kill,
		   or a slot whose OBJ_DEAD was cleared on reactivation without a body
		   reload), PtrBody[-1] hands a garbage pointer to SetInterDepObjet ->
		   data abort at a tiny address (guru addr ~0x0C). Gate on Body too,
		   matching InitAnim's invariant: a bodyless object simply isn't
		   animated this frame (correct — it is invisible/dead anyway). */
		/* PORT: the two pointer tests are part of the SAME condition on
		   purpose. HQR_Get returns 0 when an entry cannot be loaded (now
		   also when it is larger than its pool, see HQ_RESS.C), and handing
		   that to SetInterDepObjet dereferences it at once -> data abort at
		   a tiny address. Folding the checks in here makes such an object
		   take the identical path to a bodyless one — not animated this
		   frame, but the collision tail below still runs — instead of an
		   early return that would skip it. */
		if (APtObj->Anim != -1 && APtObj->Body != -1 &&
			(ptranim = HQR_Get(HQR_Anims, APtObj->Anim)) != 0 &&
			(ptrbody = PtrBody[APtObj->Body]) != 0)
		{
			numanim = APtObj->Anim;
	...
	if (Nyw < 0)
		Nyw = 0;
	if (Nxw > 63 * SIZE_BRICK_XZ)
		Nxw = 63 * SIZE_BRICK_XZ;
	if (Nzw > 63 * SIZE_BRICK_XZ)
		Nzw = 63 * SIZE_BRICK_XZ;
	...
	case MOVE_RANDOM:
		if (!(ptrobj->WorkFlags & ANIM_MASTER_ROT))
		{
			if (ptrobj->Col & 128)
			{
				...
				LE_W32(&ptrobj->Info, TimerRef + Rnd(300) + 300); /* PORT (edit #33): 32-bit timer punned over Info/Info1; offsetof(T_OBJET,Info) is 2 mod 4 — a raw ULONG store on ARM ignores the low addr bits and stomps OffsetLife */
				InitAnim(GEN_ANIM_RIEN, ANIM_REPEAT, NO_ANIM, numobj);
			}
			if (!ptrobj->RealAngle.TimeValue)
			{
				InitAnim(GEN_ANIM_MARCHE, ANIM_REPEAT, NO_ANIM, numobj);
				if (TimerRef > (ULONG)LE_R32(&ptrobj->Info)) /* PORT (edit #33) */
				{
					...
					LE_W32(&ptrobj->Info, TimerRef + Rnd(300) + 300); /* PORT (edit #33) */
				}
			}
		}
		break;
...
void AffScene(LONG flagflip)
{
	...
	for (n = 0; n < NbObjets; n++, ptrobj++)
	{
				...
				AND(!(ptrobj->Flags & INVISIBLE)))
		{
			...
			//			if( (Xp>VIEW_X0) AND (Xp<VIEW_X1) AND (Yp>VIEW_Y0) AND (Yp<VIEW_Y1) )
			{
				ztri = ptrobj->PosObjX - WorldXCube + ptrobj->PosObjZ - WorldZCube;
				...
				{
					ztri = ListObjet[num].PosObjX - WorldXCube + ListObjet[num].PosObjZ - WorldZCube + 2;
				}
				...
				if (ptrobj->Flags & SPRITE_3D)
				{
					...
					if (ptrobj->Flags & SPRITE_CLIP)
					{
						ztri = ptrobj->AnimStepX - WorldXCube + ptrobj->AnimStepZ - WorldZCube;
					}
				}
				else
				{
...
void CheckZoneSce(T_OBJET *ptrobj, WORD numobj)
{
																		 ...
																		 AND z <= ptrz->Z1)
		{
			...
			case 1: // set pos camera
				if (numobj == NumObjFollow)
				{
					...
					if (StartXCube != ptrz->Info0 OR StartYCube != ptrz->Info1 OR StartZCube != ptrz->Info2)
					{
	...
	if (flaggrm == FALSE AND numobj == NumObjFollow AND ZoneGrm != -1)
	{
		IndexGrm = ZoneGrm = -1;
		CopyMapToCube();
		FirstTime = TRUE;
