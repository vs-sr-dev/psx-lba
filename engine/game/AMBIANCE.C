...
void PlayMidiFile(WORD num)
{
			...
			OR(!IsMidiPlaying()))
	{
		...
		/* PORT: HQR_Midi is NULL on the DS (see PERSO.C) — the platform
		 * PlayMidi() reads NumXmi and plays a pre-rendered track, so the
		 * XMI blob is never used. */
		PtrXmi = HQR_Midi ? HQR_Get(HQR_Midi, num) : 0;
		NumXmi = num;
		PlayMidi(PtrXmi);
		VolumeMidi(100);
	}
...
void PlayAllMusic(WORD num)
{
	if (Midi_Driver_Enable)
	{
		...
		{
			StopMusicMidi();
			PtrXmi = HQR_Midi ? HQR_Get(HQR_Midi, num) : 0; /* PORT: see above */
			NumXmi = num;
			PlayMidi(PtrXmi);
			VolumeMidi(100);
		}
