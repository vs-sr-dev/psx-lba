				...
				WORD num, WORD nb)
{
	...
	for (n = 0; n < MAX_EXTRAS; n++, ptrextra++)
	{
		...
		ptrextra->Flags = EXTRA_STOP_COL
						  //				+ EXTRA_WAIT_NO_COL
						  + EXTRA_WAIT_SOME_TIME + EXTRA_TAKABLE;

		/* Special for Key ( Beurk cause detect with sprite number ! ) */

		if (num != 6)
			ptrextra->Flags += EXTRA_TIME_OUT + EXTRA_FLASH;
				 ...
				 WORD numobj, WORD speed, WORD hitforce)
{
	...
	for (n = 0; n < MAX_EXTRAS; n++, ptrextra++)
	{
		...
		*/
		...
		/* PORT: homing speed lives in Divers, NOT Vz. The (T_REAL_VALUE *)
		   &OrgX overlay is 12 bytes on ARM/ILP32 (ULONG MemoTicks is 4-aligned
		   -> struct padded to 12), not the 10 bytes the DOS/Watcom-packed
		   layout assumed, so its MemoTicks lands on Vy+Vz and InitRealValue
		   clobbers Vz. Re-inited from Vz every frame, the speed collapsed to
		   (TimerRef>>16)~0 and the returning ball just sat still. Divers
		   (offset 32) is outside the overlay and free for SEARCH balls. */
		ptrextra->Divers = speed;
					...
					WORD num, WORD numobj)
{
	...
	for (n = 0; n < MAX_EXTRAS; n++, ptrextra++)
	{
		...
		/* PORT: speed in Divers, not Vz — Vz is clobbered by the T_REAL_VALUE
		   overlay on ARM/ILP32 (see ExtraSearch for the full explanation). */
		ptrextra->Divers = 4000;
		ptrextra->HitForce = 0;
						...
						WORD newx, WORD newy, WORD newz)
{
	...
	if (WorldColBrick(newx, newy, newz))
		return 1;
...
void InitSpecial(WORD x, WORD y, WORD z, WORD num)
{
	...
	for (n = 0; n < MAX_EXTRAS; n++, ptrextra++)
	{
		...
		case 0: /* small star */
			...
			ptrextra->Flags = EXTRA_TIME_OUT + EXTRA_END_COL;
...
LONG CoulRetourBalle(T_EXTRA *ptrextra)
{
	...
	if (ptrextra->Sprite == 42)
		sprite = 109; // vert
	if (ptrextra->Sprite == 43)
		sprite = 110; // rouge
				...
				WORD hitforce)
{
	...
	for (n = 0; n < MAX_EXTRAS; n++, ptrextra++)
	{
		...
		ptrextra->Flags = EXTRA_END_OBJ + EXTRA_END_COL + EXTRA_WAIT_NO_COL + EXTRA_IMPACT;
...
void GiveExtraBonus(T_OBJET *ptrobj)
{
	...
	if ((MagicLevel == 0) AND(n == 2))
		n = 1; // si pas magie -> coeur
...
void GereExtras()
{
	...
	for (n = 0; n < MAX_EXTRAS; n++, ptrextra++)
	{
		...
		if (ptrextra->Flags & EXTRA_FLY)
		{
			...
			ptrextra->PosY = ptrextra->Vy * time + ptrextra->OrgY - (ptrextra->Poids * time * time) / 16;
		...
		if (ptrextra->Flags & EXTRA_SEARCH_OBJ)
		{
			...
			InitRealValue(0, ptrextra->Divers, /* PORT: speed store moved off Vz */
