...
#else
#define CSV_FILE_PATH "assets\\text\\"
#endif
...
char *GetCustomizedMultiText(LONG numParam)
{
    ...
    /* PORT: was `return result ? result : "";` — the documented contract is
     * that the CALLER frees the result (GetMenuMultiTextAux and
     * InfoWallCollisionDamage both do), and free() of the .rodata literal ""
     * corrupts the heap: msvcrt silently ignored the foreign pointer, newlib
     * on the DS walks ASCII rodata as chunk headers and data-aborts inside
     * _free_r (crash on the pause menu: texts 950/951 are not in TEXT.HQR,
     * and no ctxt.csv is shipped, so this fallback runs for every menu
     * entry the HQR lacks). Return a freeable empty string instead. */
    return result ? result : (char *)calloc(1, sizeof(char));
}
