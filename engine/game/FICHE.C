...
void GereAnimAction(T_OBJET *ptrobj, WORD numobj)
{
	...
	/* PORT: l'anim en cours n'a pas forcement un bloc d'action — SearchAnim
	   laisse SearchPtrAnimAction a 0 dans ce cas. Les appelants testent, mais
	   plutot une ligne de log qu'un deref de 0 si un chemin oublie le test. */
	if (ptrc == 0)
	{
		PORT_Diag("[ANM] GereAnimAction: obj %d anim %d has no action block\n",
				  (int)numobj, (int)ptrobj->Anim);
		return;
	}
	...
	for (n = 0; n < nb; n++)
	{
		...
		case 6: /* sample */
			...
			{
				HQ_3D_MixSample(LE_R16(ptrc), 0x1000, 1,
		...
		case 1:				   /* BODY */
			...
			if (numbody == numgene) // ok trouve
			{
				...
				else
				{
					...
					*/
					if (!PtrBody[NbBodys])
						TheEnd(NOT_ENOUGH_MEM, "Body.HQR in HQ_Mem");
					...
					LE_W16(ptrc, hqrbody + (1 << 15)); /* PORT: odd offset — LE access */
				}
				ptrc += 2;
