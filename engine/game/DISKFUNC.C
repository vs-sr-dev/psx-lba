...
#define GET_WORD       \
	(LE_R16(PtrSce));  \
	PtrSce += 2 /* PORT: scene stream has odd offsets — LE access */
...
WORD LoadScene(WORD numscene)
{
	...
	/* PORT: T_ZONE/T_TRACK are read as structs straight from the stream;
	 * after the variable-size track/life blocks PtrSce can be odd (fine on
	 * x86, fatal on ARM). Slide the tail of the scene entry down one byte. */
	if ((ULONG)PtrSce & 1)
	{
		ULONG used = (ULONG)(PtrSce - PtrScene);
		ULONG total = Size_HQR(PATH_RESSOURCE "scene.hqr", numscene);

		if (total > used)
			memmove(PtrSce - 1, PtrSce, total - used);
		PtrSce--;
	}
