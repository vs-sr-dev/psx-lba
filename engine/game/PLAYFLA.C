...
#pragma pack(push, 1) /* PORT: FLI/FLA on-disk structures — must match file layout exactly */
typedef struct
{
	LONG size;
...
#pragma pack(pop) /* PORT: end on-disk structures */
		...
		case FLA_INFO:
			...
			case 1: // fla flute
				if (Midi_Driver_Enable)
				{
					...
					{
						StopMusicMidi();
						PtrXmi = HQR_Midi ? HQR_Get(HQR_Midi, 26) : 0; /* PORT: see AMBIANCE.C */
						NumXmi = 26;
						PlayMidi(PtrXmi);
						VolumeMidi(100);
					}
...
WORD InitFla(char *name)
{
	...
	if (!HandleFla)
		return FALSE; /* PORT: no .FLA shipped (GOG HD/nitroFS); newlib fread(NULL) crashes where msvcrt returned 0 */
